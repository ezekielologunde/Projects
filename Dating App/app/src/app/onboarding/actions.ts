...
export async function processUploadedImage(ticketId: string) {
  ...
  // at the same position, with the secret key. Best-effort: a failure here
  // is cleaned up later by the purge_incoming cron job (spec section 7.19),
  // not a reason to fail the whole request after the row is already correct.
