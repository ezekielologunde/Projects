...
- Exit: a new user can complete onboarding and be approved by an aal2-enrolled admin; pgTAP covers every policy in this phase, including the `profile_sensitive` split, the admin MFA gate, the private-schema unreachability test for every helper introduced so far, and the four cron jobs above.
