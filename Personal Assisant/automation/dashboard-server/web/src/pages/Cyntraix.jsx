...
const MODULE = NAV_ITEMS.find((n) => n.key === 'cyntraix')
...
export default function Cyntraix() {
  ...
  if (!c) return <div className="page"><PageHeader /><div className="empty">not set up yet.</div></div>

  return (
    <div className="page">
      <PageHeader />
      <Card title={`${c.business.name} — ${c.business.role}`} sub={`founded ${c.business.founded} · ${c.business.structure}`} accentColor={MODULE.color} />
      {c.clients.map((cl, i) => (
        ...
        <Card key={`${cl.name}-${i}`} title={cl.name} badge={<span className={cl.status === 'active' ? 'pulse' : undefined}>{cl.status}</span>} accentColor={MODULE.color}>
