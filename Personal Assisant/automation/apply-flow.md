...
The local dashboard (`automation\dashboard-server\`) reads live from disk on every page load — no rebuild step needed; the updated `status.json` files are reflected automatically next time the dashboard is open or refreshed.
